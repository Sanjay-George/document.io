...
export default function Form({ projectId, documentationId, postSubmit }: { projectId: string, documentationId: string | null, postSubmit: (data?: any) => void }) {
    ...
    return (
        <form className="max-w-sm mx-auto" onSubmit={handleSubmit}>
            ...
            <div className="mb-8">
                <label htmlFor="url" className="block mb-2 text-sm font-medium text-gray-900 ">Base URL</label>
                <input value={formData.url} type="url" id="url" name="url" onChange={handleTextChange} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block w-full p-2.5       " placeholder="eg: https://www.google.com/" required />
            </div>

            <ButtonPrimary text="Save" icon={null} />
            {/* <button type="submit" className="text-white bg-emerald-700 hover:bg-emerald-800 focus:ring-4 focus:outline-none focus:ring-emerald-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center   ">Save</button> */}
        </form>
    );
}
